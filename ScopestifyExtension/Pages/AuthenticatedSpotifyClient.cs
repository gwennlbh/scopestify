...
class AuthenticatedSpotifyClient
{
    ...
    {
        return Environment.GetFolderPath(Environment.SpecialFolder.UserProfile) + "\\.scopestify\\secrets.json";
    }
    ...
    public static SpotifyClient Get()
    {
        ...
        try
        {
            ...
            var secrets = System.Text.Json.JsonDocument.Parse(
    ...
    public async static Task<(PrivateUser? user, string errorMessage)> LogIn()
    {
        ...
        authServer = new EmbedIOAuthServer(AuthenticatedSpotifyClient.callbackUri, AuthenticatedSpotifyClient.callbackUri.Port);
        ...
        {
            Scope = [Scopes.UserFollowModify, Scopes.UserLibraryRead, Scopes.UserLibraryModify, Scopes.UserFollowRead, Scopes.PlaylistModifyPublic, Scopes.PlaylistModifyPrivate, Scopes.UserReadPlaybackState],
        };
...
clientId, clientSecret, response.Code, AuthenticatedSpotifyClient.callbackUri
          )
        );
